...
def evaluate_mvrm_general_viral(to_predict_csv):
    # 1. load & filter viral
    df    = pd.read_csv(to_predict_csv)
    viral = df[df["Dataset"].str.contains("Viral", na=False)].copy()
    ...
    tg_seq  = seqs.tolist()
    tg_name = viral["RNA_Target"].tolist()
    
    # 3. Mordred descriptors + GeometricalShapeIndex
    calc       = Calculator(descriptors, ignore_3D=True)
    mols       = [Chem.MolFromSmiles(smi) for smi in viral["SMILES"]]
    mordred_df = calc.pandas(mols)
...
evaluate_mvrm_riboswitches("data/task7/to_predict.csv", "data/author-performance/Physicochemical_indices_RNA.csv")
evaluate_mvrm_mirna("data/task7/to_predict.csv")
evaluate_mvrm_aptamer("data/task7/to_predict.csv")
evaluate_mvrm_general_viral("data/task7/to_predict.csv")
